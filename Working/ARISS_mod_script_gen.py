...
| VERSION: 4.0.0
| STATUS: Released.
| SPDX-FileCopyrightText: 2026 Ken McCaughey
...
outline_report_file_prefix = 'ARISS_mod_script_'  
outline_report_file_suffix = '_outline'
...
a_dictionary = {
    ...
    'Conference start, event local time (HH:mm)': '12:00',
    'ISS rise predict, event local time (HH:mm)': '13:00',
    'Conference start, UTC time (HH:mm)': '17:00',
    'ISS rise predict, UTC time (HH:mm)': '18:00',
    # School/Group Information
    'School/group name': '{{group_name}}',
    'School/group location': '{{group_city_state}}',
    'Coordinator/teacher at event': '{{coordinator_name}}',
    ...
    'Ground station operator phone number': '{{gnd_sta_op_phone}}',
    ...
    'Contact from event\'s perspective (Yes/No)': 'Yes',
...
def read_form_file():
    ...
    for line in a_file:  # Loop through eac line in text file.
        ...
        if not (line.startswith('#') or line.startswith('\n') or line.startswith(' ')):
            ...
            except ValueError:  # Will get an error if missing : or space
                print()
                print('ERROR - Check form file for -->', line, ' ')
                print('  Check for blank variables.')
                print('  Check for proper date and time formatting.')
                print('  Check for erroneous text or comments missing leading "#".')
                print('  Check field name spelling, for missing colon, or missing space after colon.')
                print()
                print('      Field_name: Variable  <-- Should not be blank')
                print('                ^^')
                print('                ||')
                print('                |+--- must have a space afer colon')
                print('                +---- must have a colon')
                print()
                print('  Use', blank_form_file, 'as a reference.')
                print()
                sys.exit(2)
            a_dictionary[key] = value.strip('\n')  # Load dictionary. Remove newlines from values.
    # Debug code for testing the dictionary.
...
def time_check():
    """
    Description:
      - Check to see if conference time is after ISS rise time.
    Output:  
      - Report error message.
    """
    print ('  Conf start time:', Conf_time)
    print ('  ISS rise time:', AOS_time)
    if (AOS_time < Conf_time):
        print('')
        print('TIME ERROR: ISS rise is before Conference start time!')
        print('  Make sure form file times are in 24 hour format, HH:mm.')
        print('  Conference start time is typically an hour prior to ISS rise.')
        sys.exit(3)
    elif (D05<0):
        print('')
        print('TIME ERROR: Not enough time for all event blocks!')
        print('  Move up the Conference start time, typically an hour prior to ISS rise.')
        sys.exit(4)  
    else:
        print("  Time check: Good")     

def make_output_filename(file_prefix, file_suffix, ext):
    """
    Description:
    ...
    file_name = file_prefix + a_dictionary['Short name'] + '_V'\
                + a_dictionary['Script version'] + file_suffix + ext
    return file_name
...
def time_calcs():
    ...
    d03 = a_dictionary['D03']  # Default = 7. Contact preparation checklist.
    d04 = a_dictionary['D04']  # Default = 10. Run through with all questioners and ground station.
    ...
    d09 = a_dictionary['D09']  # Default = 4. Video of ARISS contact from ISS perspective.
    ...
    a_dictionary['Contact from event\'s perspective (Yes/No)'] = \
                          a_dictionary['Contact from event\'s perspective (Yes/No)'].upper()
    a_dictionary['Contact from the ISS perspective (Yes/No)'] = \
                          a_dictionary['Contact from the ISS perspective (Yes/No)'].upper()

    if a_dictionary['Contact from event\'s perspective (Yes/No)'] == 'NO':
        d08 = 0
    ...
    aos_date = a_dictionary['Date of contact (YYYY-MM-DD)']  # Get from form file.
    aos_time = a_dictionary['ISS rise predict, event local time (HH:mm)']  # Get from form file.
    ...
    try:
        ...
        exit(1)
    conf_time = a_dictionary['Conference start, event local time (HH:mm)']  # Get from form file.
    ...
    try:
        ...
        print('ERROR - Conference start, event local time (HH:mm) in form file has incorrect format.')
        exit(1)
...
def make_outline_report_file(d):
    ...
    report_text = ['',
                   ...
                   '',
                   'Event time zone abv: ' + a_dictionary['Event time zone abbreviation (CAPS)'] \
                                           + ', UTC ' + a_dictionary['Event time zone shift to UTC (+/-HH)'],
                   'Conference start at: ' + Conf_start_time + ' ' \
                                           + a_dictionary['Event time zone abbreviation (CAPS)'] \
                                           + ' (' + a_dictionary['Conference start, UTC time (HH:mm)'] \
                                           + ' UTC)',
                   'ISS rise time (AOS): ' + a_dictionary['Date of contact (YYYY-MM-DD)'] \
                                           + ' ' + AOS_time + ' ' + a_dictionary['Event time zone abbreviation (CAPS)'] \
                                           + ' (' + a_dictionary['ISS rise predict, UTC time (HH:mm)'] \
                                           + ' UTC)',
                   'Ground station info: ' + a_dictionary['Ground station callsign'] + ', in ' \
                                           + a_dictionary['Ground station location'],
                   '',
                   ...
                   T04 + ' ' + space(D04) + 'm  ' + space(R04) + 'm   #4 - Run through with all questioners & ground station',
                   ...
                   '',
                   'Any needed event site customization or additions for the event should',
                   'take place prior to Event #1, within the time constraints of Event #5,',
                   'and/or after Event #13.',
                   '',
...
def build_output_dictionary():
    ...
    context = {
        ...
        # Short name
        'short_name': a_dictionary['Short name'],
        ...
        'etz': a_dictionary['Event time zone abbreviation (CAPS)'],
        'TZ_shift': a_dictionary['Event time zone shift to UTC (+/-HH)'],
        'conf_UTC': a_dictionary['Conference start, UTC time (HH:mm)'],
        'conf_etz': a_dictionary['Conference start, event local time (HH:mm)'],
        'AOS_UTC': a_dictionary['ISS rise predict, UTC time (HH:mm)'],
        'AOS_etz': a_dictionary['ISS rise predict, event local time (HH:mm)'],
        # School info from form file
        'group_name': a_dictionary['School/group name'],
        'group_city_state': a_dictionary['School/group location'],
        'coordinator_name': a_dictionary['Coordinator/teacher at event'],
        'event_phone': a_dictionary['Event phone number'],
        'coordinator_backup_phone': a_dictionary['Emergency back-up phone number'],
        ...
        # ARISS ground station/telebridge info from form file
        'gnd_sta_callsign': a_dictionary['Ground station callsign'],
        'gnd_sta_location': a_dictionary['Ground station location'],
        'gnd_sta_phone': a_dictionary['Ground station telephone number'],
        'gnd_sta_op_name': a_dictionary['Ground station operator name'],
        'gnd_sta_op_callsign': a_dictionary['Ground station operator callsign'],
        'gnd_sta_op_phone': a_dictionary['Ground station operator phone number'],
        'support_names_callsigns': a_dictionary['Ground station support name(s) and callsign(s)'],
        'gnd_sta_audio': a_dictionary['Ground station audio interface'],
        'gnd_sta_video': a_dictionary['Ground station video interface'],
        # ARISS optional videos
        'ARRL_video': a_dictionary['Contact from event\'s perspective (Yes/No)'],
...
def make_mod_script_file(context):
    ...
    try:
        ...
        print()
        print('ERROR -', template_file, 'is missing or filename incorrect.')
        print()
        sys.exit(3)
...
def make_blank_form_file():
    ...
    """
    # NOTE: IF ANY OF THESE CHANGE, THE DICTIONARY AND THE CONTEXT
    #   BOTH NEED TO CHANGE AND MATCH.
    form_text = [
        ...
        '#   Do NOT add or remove lines, except for comments or',
        '#       valid field/variable pairs (see README disctionary secion).',
        ...
        'Script version: 1',
        '',
        '# Short one/two word nickname to be added to the output filenames.',
        '#   Limit to less than 20 characters, NO spaces.',
        'Short name: test',
        '',
        '# =========================',
        '# Important Dates and Times',
        ...
        'Date of contact (YYYY-MM-DD): ' + version_date,
        '',
        '# Conference start typically 1 hour before ISS Rise & AOS (24hr format).',
        '# Round down ISS rise time to the nearest minute (24hr format).',
        'Conference start, event local time (HH:mm): 12:00',
        'ISS rise predict, event local time (HH:mm): 13:00',
        'Conference start, UTC time (HH:mm): 17:00',
        'ISS rise predict, UTC time (HH:mm): 18:00',
        '',
        '# Event site time zone abbreviation in all CAPS, i.e. EST or PDT.',
        'Event time zone abbreviation (CAPS): {{etz}}',
        '# Event site UTC time zone offset. Prefix with + or -.',
        'Event time zone shift to UTC (+/-HH): -5',
        '',
        '# ========================',
        '# School/Group Information',
        '# ========================',
        ...
        '',
        '# ===============================',
        '# ARISS Telebridge/Ground Station',
        '# ===============================',
        'Ground station callsign: {{gnd_sta_callsign}}',
        'Ground station location: {{gnd_sta_location}}',
        'Ground station operator name: {{gnd_sta_op_name}}',
        'Ground station operator callsign: {{gnd_sta_op_callsign}}',
        '',
        '# Ground station audio interface.',
        '#    Examples: Local only, Verizon, Zoom Dial-in, Zoom,',
        '#              TEAMS, TEAMS Dial-in, WebEx, WebEx Dial-in etc.',
        'Ground station audio interface: {{gnd_sta_audio}}',
        '',
        '# Ground station video interface.',
        '#    Examples: Local only, None, Zoom, Google Meet, TEAMS, etc.',
        'Ground station video interface: {{gnd_sta_video}}',
        '',
        '# =====================',
        '# ARISS Optional Videos',
        ...
        '#    All durations are integer minutes. ONLY change if necessary.',
        '#    If adding translations to another language then  increase',
        '#    the durations of event 7 through 13.',
        '',
        ...
        'D03: 7',
        '',
        '# Event 4 - Run through with all questioners and ground station.',
...
script_file = make_output_filename(script_file_prefix, \
                                   script_file_suffix, '.docx')
outline_report_file = make_output_filename(outline_report_file_prefix, \
                                           outline_report_file_suffix,'.txt')
